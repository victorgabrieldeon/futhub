...
> `./.opencode/get-shit-done/USER-PROFILE.md` exists). Skip the read
...
```bash
PROFILE_CONTENT=$(cat "./.opencode/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `task()` in parallel:
   ...
   ```
   @gsd-advisor-researcher "First, read @./.opencode/agents/gsd-advisor-researcher.md for your role and instructions.
