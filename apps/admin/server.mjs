...
if (!apiUrl) throw new Error('ADMIN_API_URL is required.');

const app = Fastify({ bodyLimit: 10 * 1024 * 1024, logger: true });
...
app.all('/v1/admin', proxyAdminRequest);
app.all('/v1/admin/*', proxyAdminRequest);
app.get('/*', (_request, reply) => reply.type('text/html').sendFile('index.html'));

await app.listen({ host: '0.0.0.0', port: Number(process.env.PORT ?? 3000) });

async function proxyAdminRequest(request, reply) {
