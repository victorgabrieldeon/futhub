...
import { formatCardPurchase, formatPackPurchase } from '../format.js';
import { openStorePanel } from '../panel.js';
...
export default class LojaCommand extends FutHubCommand {
  async run(context: CommandContext<typeof options>): Promise<void> {
    if (!context.options.aba) {
      await this.respond(context, 'loja', () => openStorePanel(context.author.id));
      return;
    }
